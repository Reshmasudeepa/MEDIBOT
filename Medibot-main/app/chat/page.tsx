  ...
  useEffect(() => {
    ...
    const modelMap: Record<string, { api: string; model: string; key: string; plan: 'free' | 'premium' }> = {
      ...
      "gemini-2.0-flash": {
        ...
        key: process.env.NEXT_PUBLIC_GEMINI_API_KEY || "AIzaSyBYY_bUTdB_ixOkV8wmgkx5Zd-sErId3jQ",
        plan: "premium"
      },
      "gpt-4o": {
        api: "openai",
    ...
    try {
      ...
      const modelMap: Record<string, { api: string; model: string; key: string; plan: 'free' | 'premium' }> = {
        ...
        "medibot": {
          api: "xai",
          model: "grok-beta",
          key: process.env.NEXT_PUBLIC_GROK_API_KEY || "",
          plan: "free"
        },
      };
      ...
      // ✅ OpenAI / Groq / XAI
        ...
        const response = await fetch(url, {
          ...
          body: JSON.stringify({
            ...
            temperature: 0.7,
            stream: false, // Explicitly disable streaming for xAI compatibility
            max_tokens: 8192,
            top_p: 0.95,
            frequency_penalty: 0,
            presence_penalty: 0,
                  ...
                  {isValidImageUrl(msg.image) ? (
                    <div className="mb-2">
                      <Image
                        ...
                        unoptimized // Bypass Next.js server optimization to prevent 504 timeouts
                ...
                <div className="space-y-4 overflow-y-auto max-h-96">
                  {sessions.length > 0 ? (
                    sessions.map((session) => (
                      <div
                        key={session.id}
                        className={`p-4 rounded-xl border border-gray-200 dark:border-gray-700 cursor-pointer transition-colors ${currentSession?.id === session.id
